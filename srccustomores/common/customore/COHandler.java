...
import customore.generator.COWorldGenEmpty;
...
public class COHandler {
	...
	private static void patchBiomeDecorator(BiomeDecorator decorator)
	{
		...
		try
		{
			...
			decorator.coalGen =  new COWorldGenEmpty(decorator.coalGen);
			decorator.ironGen =  new COWorldGenEmpty(decorator.ironGen);
			decorator.goldGen =  new COWorldGenEmpty(decorator.goldGen);
			decorator.redstoneGen =  new COWorldGenEmpty(decorator.redstoneGen);
			decorator.diamondGen =  new COWorldGenEmpty(decorator.diamondGen);
			decorator.lapisGen =  new COWorldGenEmpty(decorator.lapisGen);
			
		}
		catch (Exception var7)
		{
			COLogger.log.throwing("CustomOreGenBase", "patchBiomeDecorator", var7);
		}
