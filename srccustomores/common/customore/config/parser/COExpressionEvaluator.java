...
public class COExpressionEvaluator
{
    ...
    public static class EvaluationDelegate
    {
        ...
        protected Object evaluate(Token token, Stack argumentStack) throws EmptyStackException, EvaluatorException
        {
            ...
            for (int ex = argValues.length - 1; ex >= 0; --ex)
            {
                ...
                {
                    argValues[ex] = COExpressionEvaluator.evaluateString(argumentStack);
                }
                else if (!argTypes[ex].isAssignableFrom(Double.class) && !argTypes[ex].isAssignableFrom(Double.TYPE))
                {
                    ...
                    {
                        argValues[ex] = Boolean.valueOf(COExpressionEvaluator.evaluateBoolean(argumentStack));
                    }
                    else
                    {
                        argValues[ex] = COExpressionEvaluator.evaluate(argumentStack);
                    }
                }
                else
                {
                    argValues[ex] = Double.valueOf(COExpressionEvaluator.evaluateNumber(argumentStack));
                }
            }
