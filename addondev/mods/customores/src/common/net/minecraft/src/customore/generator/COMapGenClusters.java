...
public class COMapGenClusters extends COMapGenOreDistribution
{
    @DistributionSetting(
            name = "Size",
            info = "Roughly the number of blocks in every deposit.  No range."
    )
    public final COPDist clSize = new COPDist(8.0F, 0.0F);
    @DistributionSetting(
            name = "Frequency",
            info = "Number of deposits per 16x16 chunk.  No range."
    )
    public final COPDist clFreq;
    @DistributionSetting(
            name = "Height",
            info = "Vertical height of the deposits.  Normal distributions are approximated."
    )
    public final COPDist clHeight;
    protected static final CODistributionSettingMap _clusterSettingsMap = new CODistributionSettingMap(COMapGenClusters.class);

    public COMapGenClusters(int distributionID, boolean canGenerate)
    {
        super(_clusterSettingsMap, distributionID, canGenerate);
        this.clFreq = this.frequency;
        this.clHeight = new COPDist(64.0F, 64.0F, Type.uniform);
