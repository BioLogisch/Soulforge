...
import net.minecraft.src.customore.config.parser.COConfigParser.ConfigExpressionEvaluator;

public class COValidatorNode
{
    private COConfigParser _parser = null;
    private Node _node = null;
    private Hashtable<List,IValidatorFactory> _validatorMap = null;
    private boolean _validatorMapShared = false;

    public COValidatorNode(COConfigParser parser, Node node)
    {
        this._parser = parser;
        this._node = node;
    ...
    protected COValidatorNode(COValidatorNode parent, Node node)
    {
    ...
    public final COConfigParser getParser()
    {
        return this._parser;
    }
    ...
    public final void validate() throws COParserException
    {
    ...
    protected boolean validateChildren() throws COParserException
    {
        if (this._validatorMap != null)
        {
            ...
            for (Node child : childList) {
                ...
                if (factory != null)
                {
                    COValidatorNode validator = factory.createValidator(this, child);
    ...
    protected void checkChildrenValid() throws COParserException
    {
        NamedNodeMap attributes = this._node.getAttributes();
        ...
        for (int children = 0; attributes != null && children < attributes.getLength(); ++children)
        {
            ...
            {
                throw new COParserException("Unexpected at this location.", c);
            }
        }
        ...
        for (int var6 = 0; var5 != null && var6 < var5.getLength(); ++var6)
        {
            ...
            switch (child.getNodeType())
            {
                ...
                default:
                    ...
                    {
                        throw new COParserException("Unexpected at this location.", child);
                    }
            }
        }
    }

    protected final <T extends COValidatorNode> LinkedList<T> validateNamedChildren(int nodeTypeMask, String nodeName, IValidatorFactory<T> factory) throws COParserException
    {
        ...
        for(COValidatorNode child : childList) {
        	child.validate();
        }
    ...
    protected final <T> T validateNamedAttribute(Class<T> attrType, String attrName, T defaultValue, boolean allowElements) throws COParserException
    {
        ...
        LinkedList<COValidatorSimpleNode> children = this.validateNamedChildren(mask, attrName, new COValidatorSimpleNode.Factory(attrType, evaluator));
    ...
    protected final <T> T validateRequiredAttribute(Class<T> attrType, String attrName, boolean allowElements) throws COParserException
    {
        ...
        {
            throw new COParserException("Required attribute \'" + attrName + "\' not found.", this.getNode());
        }
    }

    protected final void replaceWithNode(Node ... newNodes) throws COParserException
    {
            	...
            	for (Node newNode : newNodes) 
                {
            		if (newNode != null)
                    {
                        ...
                        if (newNode.getNodeType() == 2)
                        {
                            ...
                            {
                                throw new COParserException("Attempting to merge attribute to non-element node.", newNode);
                            }
    ...
    protected final void replaceWithNodeContents(Node ... containerNodes) throws COParserException
    {
        Node parent = this.getNode().getParentNode();
    ...
    public static interface IValidatorFactory<T extends COValidatorNode>
    {
        T createValidator(COValidatorNode parent, Node node);
    }


    public static class Factory implements IValidatorFactory
    {
    	public COValidatorNode createValidator(COValidatorNode parent, Node node)
    	{
    		return new COValidatorNode(parent, node);
    	}
    }
}
