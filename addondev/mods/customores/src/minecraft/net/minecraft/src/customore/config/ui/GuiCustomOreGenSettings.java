...
public class GuiCustomOreGenSettings extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        COConfigOption.DisplayGroup currentGroup = this._groupPanel == null ? null : this._groupPanel.getSelectedGroup();
        ...
        if (this.refreshGui >= 2)
        {
            ...
            COWorldConfig visibleGroups = null;

            while (visibleGroups == null)
            {
                try
                {
                    visibleGroups = new COWorldConfig();
                }
                catch (Exception var7)
                {
                    ...
                    visibleGroups = COWorldConfig.createEmptyConfig();
                }

                COWorldConfig.loadedOptionOverrides[0] = visibleGroups.getConfigOptions();

                if (currentGroup != null)
                {
                    COConfigOption visibleOptions = visibleGroups.getConfigOption(currentGroup.getName());

                    if (visibleOptions instanceof COConfigOption.DisplayGroup)
                    {
                        currentGroup = (COConfigOption.DisplayGroup)visibleOptions;
                    }
                }
            }
        }
        ...
        nextOption: for (COConfigOption option : COWorldConfig.loadedOptionOverrides[0]) {
        	
            if (option.getDisplayState() != null && option.getDisplayState() != COConfigOption.DisplayState.hidden)
            {
                COConfigOption.DisplayGroup group;

                if (option instanceof COConfigOption.DisplayGroup)
                {
                    ...
                    visibleGroups1.add((COConfigOption.DisplayGroup)option);
                }
                else
                {
    ...
    public class GuiGroupPanel
    {
        ...
        public GuiGroupPanel(int x, int y, int width, int height, COConfigOption.DisplayGroup selGroup, Vector groups)
        {
            this.posX = 0;
            this.posY = 0;
            this.width = 0;
            ...
            {
                COConfigOption.DisplayGroup group = c < 0 ? null : (COConfigOption.DisplayGroup)groups.get(c);
        ...
        public COConfigOption.DisplayGroup getSelectedGroup()
        {
            return this._currentGroup == null ? null : (COConfigOption.DisplayGroup)this._currentGroup.getOption();
        }
        ...
        {
            protected final COConfigOption.DisplayGroup _group;
            private final int _relX;
            private final int _relY;

            public GuiGroupButton(int id, int relX, int relY, int width, int height, String text, COConfigOption.DisplayGroup group)
            {
            ...
            public COConfigOption getOption()
            {
                return this._group;
            }
    ...
    public class GuiOptionSlot extends GuiSlot
    {
        ...
        public GuiOptionSlot(int top, int bottom, int slotHeight, Vector options)
        {
            ...
            {
                COConfigOption option = (COConfigOption)options.get(c);
                Object control = null;

                if (option instanceof COChoiceOption)
                {
                    this._optionControls.add(new GuiChoiceButton(this, c, 2 * width / 5 + 15, 0, width / 10 + 100, slotHeight - 6, (COChoiceOption)option));
                }
                else if (option instanceof CONumericOption)
                {
                    this._optionControls.add(new GuiNumericSlider(this, c, 2 * width / 5 + 15, 0, width / 10 + 100, slotHeight - 6, (CONumericOption)option));
                }
            }
        }
        ...
        protected void elementClicked(int index, boolean doubleClicked)
        {
            if (this._clickTarget != null)
            {
                ...
                if (this._clickTarget.getOption().getDisplayState() == COConfigOption.DisplayState.shown_dynamic)
                {
                    refreshGui = 2;
                }
        ...
        protected void drawSlot(int index, int slotX, int slotY, int slotH, Tessellator tess)
        {
            ...
            COConfigOption option = optCtrl.getOption();
        ...
        {
            private final COChoiceOption _choice;
            ...
            public GuiChoiceButton(GuiOptionSlot var1, int id, int x, int y, int maxWidth, int height, COChoiceOption choice)
            {
        ...
        {
            private final CONumericOption _numeric;

            final GuiOptionSlot this$1;

            public GuiNumericSlider(GuiOptionSlot var1, int id, int x, int y, int width, int height, CONumericOption numeric)
            {
