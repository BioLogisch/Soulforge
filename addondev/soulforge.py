...
#                      Patching
#==========================================================================
def apply_initial_patches(mcp_dir, soulforge_dir, src_dir, copy_files=True):
    #patch files
    print 'Applying initial patches'
    sys.stdout.flush()

    if os.path.isdir(os.path.join(soulforge_dir, 'initialpatches', 'minecraft')):
        apply_patches(mcp_dir, os.path.join(soulforge_dir, 'initialpatches', 'minecraft'), src_dir)
        
    if os.path.isdir(os.path.join(soulforge_dir, 'initialpatches', 'minecraft_server')):
        apply_patches(mcp_dir, os.path.join(soulforge_dir, 'initialpatches', 'minecraft_server'), src_dir)
        
def apply_soulforge_patches(mcp_dir, soulforge_dir, src_dir, copy_files=True):
    #patch files
    print 'Applying Soulforge patches'
    sys.stdout.flush()

    if os.path.isdir(os.path.join(soulforge_dir, 'patches', 'minecraft')):
        apply_patches(mcp_dir, os.path.join(soulforge_dir, 'patches', 'minecraft'), src_dir)
        
    if os.path.isdir(os.path.join(soulforge_dir, 'patches', 'minecraft_server')):
        apply_patches(mcp_dir, os.path.join(soulforge_dir, 'patches', 'minecraft_server'), src_dir)
        
def apply_mod_patches(mcp_dir, soulforge_dir, src_dir, copy_files=True):
    #patch files
    print 'Applying mod patches'
    sys.stdout.flush()

    for dirname in os.walk(os.path.join(soulforge_dir, 'addondev', 'mods', '.')).next()[1]:
        if os.path.isdir(os.path.join(soulforge_dir, 'addondev', 'mods', dirname, 'patches', 'minecraft')):
            apply_patches(mcp_dir, os.path.join(soulforge_dir, 'addondev', 'mods', dirname, 'patches', 'minecraft'), src_dir)
        
        if os.path.isdir(os.path.join(soulforge_dir, 'addondev', 'mods', dirname, 'patches', 'minecraft_server')):
            apply_patches(mcp_dir, os.path.join(soulforge_dir, 'addondev', 'mods', dirname, 'patches', 'minecraft_server'), src_dir)
        
def repatch_soulforge(soulforge_dir, mcp_dir):
    print 'Applying Soulforge patches'
    sys.stdout.flush()
    ...
    if os.path.isdir(os.path.join(soulforge_dir, 'patches', 'minecraft')):
        repatch(mcp_dir, os.path.join(soulforge_dir, 'patches', 'minecraft'), src_dir, orig_dir)
        
    if os.path.isdir(os.path.join(soulforge_dir, 'patches', 'minecraft_server')):
        repatch(mcp_dir, os.path.join(soulforge_dir, 'patches', 'minecraft_server'), src_dir, orig_dir)
        
def repatch_mod(soulforge_dir, mcp_dir):
    print 'Applying mod patches'
    sys.stdout.flush()
    ...
    for dirname in os.walk(os.path.join(soulforge_dir, 'addondev', 'mods', '.')).next()[1]:
        if os.path.isdir(os.path.join(soulforge_dir, 'addondev', 'mods', dirname, 'patches', 'minecraft')):
            repatch(mcp_dir, os.path.join(soulforge_dir, 'addondev', 'mods', dirname, 'patches', 'minecraft'), src_dir, orig_dir)
        
        if os.path.isdir(os.path.join(soulforge_dir, 'addondev', 'mods', dirname, 'patches', 'minecraft_server')):
            repatch(mcp_dir, os.path.join(soulforge_dir, 'addondev', 'mods', dirname, 'patches', 'minecraft_server'), src_dir, orig_dir)
...
#                      Binary Patch Setup
#==========================================================================
def copyreobfuscatedfiles(soulforge_dir, mcp_dir):
    basediff = os.path.join(soulforge_dir, 'addondev', 'basediff')
...
def unzipandcopybtw(soulforge_dir):
    basediff = os.path.join(soulforge_dir, 'addondev', 'basediff')
    ...
    copytree(os.path.join(btwtemp, 'MINECRAFT-JAR'), os.path.join(base_loc, 'MINECRAFT-JAR')) 
    copytree(os.path.join(btwtemp, 'MINECRAFT_SERVER-JAR'), os.path.join(base_loc, 'MINECRAFT_SERVER-JAR'))    
    
#==========================================================================
#                      Initial Setup
#==========================================================================    
def unzipbtw(soulforge_dir):
    filelist = os.listdir(os.path.join(soulforge_dir, 'addondev', 'btw'))
    zippath = os.path.join(soulforge_dir, 'addondev', 'btw', '\n'.join(fname for fname in filelist if fname.startswith("BTWMod") and fname.endswith('.zip')))
    
    btwtemp = os.path.join(soulforge_dir, 'addondev', 'btw', 'btwtemp')
    unzip(zippath, btwtemp)
    
def copyinitialreobfuscatedfiles(soulforge_dir, mcp_dir):
    minecraft_jar_loc = os.path.join(soulforge_dir, 'addondev', 'btw', 'btwtemp', 'MINECRAFT-JAR')
    minecraft_server_jar_loc = os.path.join(soulforge_dir, 'addondev', 'btw', 'btwtemp', 'MINECRAFT_SERVER-JAR')

    copytree(os.path.join(mcp_dir, 'reobf', 'minecraft'), minecraft_jar_loc) 
    copytree(os.path.join(mcp_dir, 'reobf', 'minecraft_server'), minecraft_server_jar_loc)
    
def rezipbtw(soulforge_dir):
    zipdir(os.path.join(soulforge_dir, 'addondev', 'btw', 'btwtemp'), os.path.join(soulforge_dir, 'addondev', 'basediff', 'btw', 'BTWMod-Soulforge.zip'))
    shutil.rmtree(os.path.join(soulforge_dir, 'addondev', 'btw', 'btwtemp'))  
    
#==========================================================================
#                      Create Dist
#==========================================================================
def movetodist(soulforge_dir):
    basediffdir = os.path.join(soulforge_dir, 'addondev', 'basediff')
    nonbtweditsdir = os.path.join(basediffdir, 'nonbtwedits')
    patchesdir = os.path.join(basediffdir, 'patches')
    binarypatcherdir = os.path.join(soulforge_dir, 'addondev', 'binarypatcher')
    distdir = os.path.join(soulforge_dir, 'addondev', 'dist')
    ...
    if (os.path.exists(nonbtweditsdir)):
    ...
    if os.path.isfile(os.path.join(soulforge_dir, 'addondev', 'dist', '.gitignore')):
        os.remove(os.path.join(soulforge_dir, 'addondev', 'dist', '.gitignore'))
...
#                     Cleanup
#==========================================================================
def cleanup(soulforge_dir, mcp_dir):
    basediff = os.path.join(soulforge_dir, 'basediff')
    ...
    mcp_src_loc = os.path.join(mcp_dir, 'src')
    mcp_temp_loc = os.path.join(mcp_dir, 'src_temp')
    ...
    if os.path.exists(mcp_src_loc):
        shutil.rmtree(mcp_src_loc)
        copytree(mcp_temp_loc, mcp_src_loc) 
